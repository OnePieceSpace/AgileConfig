    ...
    public class InitService : IHostedService
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            if (Appsettings.Cluster)
            {
                ...
                if (!string.IsNullOrEmpty(ip))
                {
                    ...
                    _serverNodeService.JoinAsync(ip, 6000, desc);
                    _logger.LogInformation($"AgileConfig node http://{ip}:6000 joined .");
                }
            }
        ...
        public async Task StopAsync(CancellationToken cancellationToken)
        {
            if (Appsettings.Cluster)
            {
                ...
                if (!string.IsNullOrEmpty(ip))
                {
                    await _serverNodeService.DeleteAsync($"http://{ip}:{6000}");
                    _logger.LogInformation($"AgileConfig node http://{ip}:6000 removed .");
                }
            }
        }
